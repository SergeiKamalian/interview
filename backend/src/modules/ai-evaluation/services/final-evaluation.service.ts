...
export class FinalEvaluationService {
  ...
  async evaluateAndPersistFinalEvaluation(
    ...
    const [questionEvaluations, interviewQuestions] = await Promise.all([
    ...
    if (questionEvaluations.length < interviewQuestions.length) {
      throw new BadRequestException({
    ...
    const scoreInputs: QuestionScoreInput[] = questionEvaluations.map(
      (evaluation) => {
    ...
    const userPrompt = buildFinalEvaluationUserPrompt({
      ...
      questionSummaries: questionEvaluations.map(
        (evaluation) =>
