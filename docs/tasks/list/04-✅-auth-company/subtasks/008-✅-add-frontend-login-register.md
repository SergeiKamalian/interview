...
**Сделано:** страницы `/login` и `/register`, GraphQL mutations, authSlice `setCredentials`, редирект на dashboard.

**Проверки:**

| Команда | Ожидание | Результат |
|---------|----------|-----------|
| `cd frontend && npm run build` | exit 0 | OK (~603ms vite build) |
| `cd frontend && npm run lint` | exit 0 | OK |
