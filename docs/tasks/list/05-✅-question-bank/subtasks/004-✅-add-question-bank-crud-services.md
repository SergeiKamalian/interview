...
- `QuestionBankRepository` + `QuestionBankService` с транзакционным create/update (replace strategy для checkpoints/examples).
- Archive = soft delete (`deleted_at`, `is_active = 0`), только company-owned вопросы.
