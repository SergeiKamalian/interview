...
После блока 06 уже существует interview flow и ответы кандидата. В блоке 07 backend вызывает LLM-провайдера, передаёт контекст вопроса из question bank, получает строго структурированный JSON, сохраняет результаты в MySQL и отдаёт данные в dashboard блок 08.

## Что входит в этот блок

- Конфигурация AI-провайдера через env и typed config service.
...
- Question bank (`question_bank_questions`, `question_bank_checkpoints`) — единственный источник критериев оценки.
- LLM возвращает только структурированный JSON, который валидируется до записи в БД.
...
Для завершенного интервью можно безопасно запустить AI-оценку: система использует checkpoints из question bank, получает валидный JSON, сохраняет все уровни оценок, считает итоговый score и пишет usage/cost лог.
